...
<head>
    ...
    <title>Purchase <?php echo $_SESSION['Monitor_Title']; ?> </title>
    ...
    <div class="container-fluid d-flex flex-column justify-content-center align-items-center">
        <div class="row d-flex col-12 justify-content-between p-1 mt-2 align-items-center">
            ...
            <div class="card col-sm-10 col-md-5 col-lg-4 ">
                ...
                $Select_Query = "SELECT * FROM `monitors Table` WHERE Id = $getid";

                $Fire_Query = mysqli_query($conn, $Select_Query);

                $Result = mysqli_fetch_array($Fire_Query);

                $Monitor_Quantity = $Result['Monitor Quantity'];
            ...
            <div class="text-center col-sm-10 col-md-6 col-lg-6 p-3">
                <h2><?php echo $Result['Monitor Title']; ?></h2>
                <h4><?php echo $Result['Monitor Description']; ?></h4>
                ...
                if (isset($_POST['Add_To_Cart'])) {
                    ...
                    $Product_Title = $Result['Monitor Title'];
                    $Product_Description = $Result['Monitor Description'];
                    $Product_Price = $_SESSION['Monitor_Price'];

                    $Total = $_SESSION['Monitor_Price'] * $Buyer_Enter_Quantity;
                ...
                if (isset($_POST['Order'])) {
                    ...
                    $Available_Quantity = $Monitor_Quantity - $Buyer_Enter_Quantity;

                    if ($Available_Quantity < 0) {
                    ?>
                        <script>
                            alert('out of stock <?php echo "Product Quantity is = ". $Monitor_Quantity ?>');
                        </script>
                        <?php
                    } else {
                        ...
                        $update = "UPDATE `monitors table` SET `Monitor Quantity`='$Available_Quantity' WHERE Id = $getid ";
                ...
                <div class="d-flex justify-content-between p-3">
                    ...
                    <?php
                    $select = "select * from `monitors table` where Id = $getid";
                    $querrn = mysqli_query($conn, $select);
                    $fetching = mysqli_fetch_array($querrn);
                    $availableqntity = $fetching['Monitor Quantity'];
                    ?>
                    <h5 class="text-center text-success bg-light rounded-2 p-1 p-md-2"> Available Quantity: <?php echo $availableqntity ?></h5>
                    <h5 class="text-center text-success bg-light rounded-2 p-1 p-md-2"><span class="">Price : <?php echo $_SESSION["Laptop_Price"]; ?></span></h5>
                </div>
